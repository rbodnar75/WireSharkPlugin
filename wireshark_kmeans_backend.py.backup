#!/Users/rbodnar/Repos/WireSharkPlugin/venv/bin/python
"""
Wireshark K-means Backend Script

This script is designed to work with the Wireshark K-means Analyzer plugin.
It performs the actual machine learning analysis and returns results in a format
that can be consumed by the Wireshark Lua plugin.
"""
...
class WiresharkKmeansAnalyzer:
    ...
    def extract_features(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            features['src_local'] = 1 if re.match(r'^(10\.|172\.16\.|192\.168\.)', src_ip) else 0
            features['dst_local'] = 1 if re.match(r'^(10\.|172\.16\.|192\.168\.)', dst_ip) else 0
...
def main():
    parser = argparse.ArgumentParser(description='Wireshark K-means Backend Analyzer')
